...
public class Add implements Command, ScriptCommand {
  ...
  public void execute() {
    ...
    if (collectionManager.add(p)) ioService.print(p.getName() + " added to collection");
    else ioService.print("Failed to add person");
  }
  ...
  public int scriptExecution(List<String> lines, int startIndex) {
    ...
    if (collectionManager.add(p)) ioService.print("Added: " + p.getName());
    else ioService.print("Failed to add: " + p.getName());
